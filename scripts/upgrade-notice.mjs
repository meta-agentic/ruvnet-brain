...
export function noticeFor(version) {
  ...
  return [
    ...
    ``,
    `  · Source-grounded answers now run through RuVector RVF knowledge stores and cite the`,
    `    underlying source instead of relying on model memory.`,
    ``,
    `  · Claude Code and Codex are first-class hosts, kept on one Stable Spine so updates,`,
    `    hooks, skills and the search runtime converge on the same active generation.`,
    ``,
    `  · The Console makes the installed stack, AgentDB learning, controls and reversible`,
    `    actions visible in one place.`,
    ``,
    `  · Agentic-QE, harness scoring and cost-aware routing are exposed as explicit workflows`,
    ...
    ``,
    `  Full ${release} highlights: npx ruvnet-brain --whats-new`,
    ``,
