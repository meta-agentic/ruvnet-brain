...
|---|---|---|
| 2026-07-29 | Windows maintenance supervision now crosses the native `cmd start` no-wait boundary; job arguments travel as a base64 JSON environment payload so user/log paths never enter a shell command. POSIX keeps the existing `setsid` path. | `windowsHide` alone did not close the inherited hook capture handle: PR #58 run `30423859369` again showed the body finished in 0.86s while the watchdog returned at 7.18s. Governed source `plugin/scripts/detach.mjs`. |
