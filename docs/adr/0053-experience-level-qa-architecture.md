...
|---|---|---|
| 2026-07-30 | Updated the cross-platform Console acceptance oracle to the shipped 4.0.2 controls and made platform-conditional absence an explicit result instead of a selector timeout. | PR #68’s exact-SHA UX jobs proved `tests/ux/render-probe.mjs` still expected only provider/advocacy plus seven unsupported controls. The product now has eight universally owned controls and a ninth nightly control only where its scheduler is supported. The probe derives that expected surface from the rendered unavailability evidence and still exercises real save/reload, Fix All, and undo paths whenever a verified recommendation exists. |
