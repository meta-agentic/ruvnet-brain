...
// kb/capability-cards.md (64→32-card zero-ML keyword lane; see kb/card-lane.mjs header for the
// full "measured 19.6s warm / 73s cold heavy path" backstory).
//
...
describe('answerFromCards — POSITIVE: a real, representative "does rUv ship X" question set', () => {
  ...
  it('measures and reports the REAL hit fraction on this representative set (no estimate)', () => {
    ...
    for (const r of results) {
      // eslint-disable-next-line no-console
      console.log(`  ${r.hit.hit.hit ? 'HIT ' : 'MISS'} repo=${r.hit.hit.repo || '-'}  "${r.q.query}"`);
    }
...
describe('answerFromCards — NEGATIVE: silence-or-fallthrough, NEVER a fabricated hit', () => {
  ...
  it('a question about a genuinely uncovered (privately-fenced) repo is not fabricated either', () => {
